...
const AddItemScreen = ({ route }: any) => {
  ...
  const { code, previousScreen, searchCode } = route.params;
  const {
    data: product,
    isLoading,
    isSuccess,
  ...
  const [isAutoGeneratedCode, setAutoGeneratedCode] = useState(
    searchCode ? false : true
  );
  ...
  const fieldValidationSchema = code
    ? yup.object().shape({
        perUnit: yup
          .number()
  ...
  useEffect(() => {
    if (code == null) {
      ...
      if (product?.items.length > 0) {
        const _product = product?.items[0];
        const hist =
          _product.priceHistories.length > 0 ? _product.priceHistories : [];

        if (hist.length > 0) {
          setItem({ ...item, perUnit: hist[hist.length - 1].price });
        }
      }
    }
                ...
                <VStack space={2}>
                  ...
                  >
                    <Input
                      ...
                      autoCapitalize={"characters"}
                      bgColor="white"
                      defaultValue={searchCode ? searchCode : ""}
                      _focus={{
                        selectionColor: "green",
                      }}
                      placeholder={`${t(
